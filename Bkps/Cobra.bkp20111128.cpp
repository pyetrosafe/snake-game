...
GLfloat start_time, end_time, clock_time, pause_time, paused_time;
GLfloat Xfood, Yfood;
...
struct cobra food;
long seed=0, counter=0, ScorePoints;
char niveltxt[100];
...
int sqXmin=0, sqXmax=0;  // Tamanho minimo e maximo em X da Janela
int sqYmin=0, sqYmax=0;  // Tamanho minimo e maximo em Y da Janela
...
FSOUND_SAMPLE* somAndou; // Som quando anda
FSOUND_SAMPLE* somComeu;  // Som quando come
FSOUND_SAMPLE* somMenu;  // Som Inicial
FSOUND_SAMPLE* somChoque;  // Som Choque
FSOUND_SAMPLE* somParabens;  // Som Choque
...
void mostraTeclas() {
    ...
    DesenhaTexto(GLUT_BITMAP_TIMES_ROMAN_24,msg,-(HRES/2)+40,(VRES/2)-20);
}
...
void Debug(){
    ...
    if (StatusGame == ON) {
      ...
      DesenhaTexto(GLUT_BITMAP_TIMES_ROMAN_24,msg,-(HRES/2)+40,(VRES/2)-30);
      sprintf(msg,"Vidas: %i",alive);
      DesenhaTexto(GLUT_BITMAP_TIMES_ROMAN_24,msg,(HRES/2)-90,(VRES/2)-20);
      sprintf(msg,"Tamanho: %i ",contrastro);
      DesenhaTexto(GLUT_BITMAP_TIMES_ROMAN_24,msg,(HRES/2)-90,(VRES/2)-30);
      sprintf(msg,"Tempo: %.0f",((clock()-clock_time)/CLOCKS_PER_SEC)-paused_time);
      DesenhaTexto(GLUT_BITMAP_TIMES_ROMAN_24,msg,(HRES/2)-90,(VRES/2)-40);
/*
...
void DesenhaTela() {
    ...
    glBegin(GL_QUADS);
    glVertex2f(sqXmin, winYmax);
    glVertex2f(sqXmin, sqYmax);
    glVertex2f(sqXmax, sqYmax);
    glVertex2f(sqXmax, winYmax);
    glEnd();

    // Barra horizontal inferior
    glBegin(GL_QUADS);
    glVertex2f(sqXmin, sqYmin);
    glVertex2f(sqXmin, winYmin);
    glVertex2f(sqXmax, winYmin);
    glVertex2f(sqXmax, sqYmin);
    glEnd();
    ...
    glVertex2f(sqXmin, sqYmin);
    glVertex2f(sqXmin, sqYmax);
    glVertex2f(winXmin, sqYmax);
    glVertex2f(winXmin, sqYmin);
    glEnd();

    // Barra vertical direita
    glBegin(GL_QUADS);
    glVertex2f(winXmax, sqYmin);
    glVertex2f(winXmax, sqYmax);
    glVertex2f(sqXmax, sqYmax);
    glVertex2f(sqXmax, sqYmin);
    glEnd();
...
void Engine(int value) {
    ...
    if(StatusGame==OFF && (Stats==4 || Stats==5)) {// Contador para tela 4 e 5(Level e Choque)
      FSOUND_StopSound(2); // Para a execução da musica inicial (id 2)
...
void Teclado(unsigned char tecla, int x, int y){
    ...
    if (tecla == '+' || tecla == '=') {
      if (FSOUND_GetSFXMasterVolume() > 242)
        FSOUND_SetSFXMasterVolume(255);
      else
        FSOUND_SetSFXMasterVolume(FSOUND_GetSFXMasterVolume()+13);
      //teclas = tecla;
    }

    if (tecla == '-') {
      if (FSOUND_GetSFXMasterVolume() < 13)
...
void Inicializa() {
   ...
   //Carrega os arquivos com os sons
    somAndou=FSOUND_Sample_Load (0,"SomAndou3.wav",0, 0, 0);
    FSOUND_Sample_SetMode(somAndou, FSOUND_LOOP_OFF);  

    somComeu=FSOUND_Sample_Load (1,"SomComeu.wav",0, 0, 0);
    FSOUND_Sample_SetMode(somComeu, FSOUND_LOOP_OFF);

    somMenu=FSOUND_Sample_Load (2,"SomMenu.mp3",0, 0, 0);
    FSOUND_Sample_SetMode(somMenu, FSOUND_LOOP_OFF);

    somChoque=FSOUND_Sample_Load (3,"SomChoque2.wav",0, 0, 0);
    FSOUND_Sample_SetMode(somChoque, FSOUND_LOOP_OFF);  

    somParabens=FSOUND_Sample_Load (4,"SomParabens1.mp3",0, 0, 0);
    FSOUND_Sample_SetMode(somParabens, FSOUND_LOOP_OFF);

    somPerdeu=FSOUND_Sample_Load (5,"SomPerdeu.wav",0, 0, 0);
